...
function measureWidths(el: HTMLElement, texts: string[]) {
  ...
  const widths = texts.map(t => { ghost.textContent = t; return ghost.getBoundingClientRect().width })
  ghost.remove()
  return widths
}
...
export function DiaTextReveal({
  text, colors = DEFAULT_COLORS, textColor = "var(--foreground)",
  duration = 1.5, delay = 0, repeat = false, repeatDelay = 0.5,
  startOnView = true, once = true, className, fixedWidth = false, ...props
}: DiaTextRevealProps) {
